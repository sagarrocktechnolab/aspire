...
class CustomersController extends Controller
{
   ...
   /* public function __construct()
	...
	public function processLogin(Request $request){
		...
		$customers_password      = md5($request->customers_password);
	...
	public function processRegistration(Request $request){
		...
		$customers_legalname           			=   $request->customers_legalname;			
		$customers_email_address    		    =   $request->customers_email_address;
		$customers_password          		    =   md5($request->customers_password);	
		...
		$rules['customers_firstname'] = 'required|string';
		$rules['customers_lastname'] = 'required|string';
		$rules['customers_legalname'] = 'required|string';
		$rules['customers_email_address'] = 'required|email|string';
		$rules['customers_password'] = 'required|string';
		$rules['customers_telephone'] = 'required|integer|min:1';		
		...
		if(count($existUser)=="1"){	
			...
			$customer_data = array(
				...
				'customers_legalname'			 =>  $customers_legalname,
	...
	public function updateCustomerInfo(Request $request){
		...
		$rules['customers_id'] = 'required|string';
		$rules['customers_firstname'] = 'required|string';
		$rules['customers_lastname'] = 'required|string';
		$rules['customers_legalname'] = 'required|string';		
		$rules['customers_password'] = 'required|string';
		...
		//check email existance
		$existUser = DB::table('customers')->where('customers_id', $customers_id)->get();	
		
		if(count($existUser) !="1"){	
			//response if email already 
			$postData = array();
			$responseData = array('success'=>'0', 'data'=>array(), 'message'=>array('error_message'=>'Customer is not available with this id.'));
			$userResponse = json_encode($responseData);
			print $userResponse;die;
		}
	...
	public function processForgotPassword(Request $request){
		...
		if(count($existUser)>0){
			...
			Mail::send('/mail/recoverPassword', ['existUser' => $existUser], function($m) use ($existUser){
				$m->to($existUser[0]->customers_email_address)->subject('Aspire Password Recovery')->getSwiftMessage()
				->getHeaders()
				->addTextHeader('x-mailgun-native-send', 'true');	
			});
